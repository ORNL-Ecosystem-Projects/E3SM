module controlMod
  ...
  use elm_varctl, only: nu_com, use_dynroot, use_fan, fan_mode, fan_to_bgc_veg, &
                        ...
                        use_shrub_moss_shading, use_surface_structure_shading
  !
  ! !PUBLIC TYPES:
  implicit none
  save
...
contains
  ...
  subroutine control_spmd()
    ...
    call mpi_bcast (use_surface_structure_shading, 1, MPI_LOGICAL, 0, mpicom, ier)
    ...
    if (use_moss_capillary_nutrients .and. .not. use_peatland_roots) then
    ...
    if (use_peatland_compaction_profile .and. .not. use_peatland_vertical_transport) then
