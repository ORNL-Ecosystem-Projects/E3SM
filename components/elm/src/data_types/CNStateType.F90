module CNStateType
  ...
  type, public :: cnstate_type
     ...
     integer  , pointer :: spinup_t_nyears_patch       (:)     ! years included in spinup reference temperature
...
contains
  ...
  subroutine Restart(this, bounds, ncid, flag)
    ...
    if (use_humhol) then
       ...
       if (flag == 'read' .and. .not. readvar) then
          this%spinup_t_patch(bounds%begp:bounds%endp) = 280._r8
       end if
       ...
       if (flag == 'read' .and. .not. readvar) then
          this%spinup_t_nyears_patch(bounds%begp:bounds%endp) = 0
       end if
    end if
