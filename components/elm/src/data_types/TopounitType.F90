module TopounitType
  ...
  type, public :: topounit_physical_properties
    ...
    real(r8), pointer :: lateral_dist (:) => null() ! lateral distance to next lower topounit (m)
    integer , pointer :: regional_target_ti (:) => null() ! target topounit for regional lateral flow
